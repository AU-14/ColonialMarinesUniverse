...
- Remaining debt: Old RMC loading also rejected primed ordnance and non-brand-new expendable lights, and generic provider-to-provider transfer batches differed from current SS14. Those are classified `Missing` and `Behavior changed` respectively and remain separate from this timing restoration. Runtime cancellation, prediction reconciliation, stack splitting, full-at-completion, dropped-ammo, and launcher/assisted reload coverage is required after the checkpoint.
