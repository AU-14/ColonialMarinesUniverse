...
## CS-0324 - Restore RMC resting speed post-processing

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0323.
- Areas: Movement, Standing/resting, Speed modifiers, Event ordering, Prediction
- Classification: Missing -> Adapted.
- Risk: Medium before the fix because `RMCRestComponent.RestingSpeed` was networked and toggled but no longer affected movement; low-to-medium after it pending modifier-order and prediction coverage.
- Behavior/API delta: Current SS14 aggregates `RefreshMovementSpeedModifiersEvent` subscribers before updating replicated movement modifiers. Live RMC applied its resting override after those subscribers, compensating for accumulated non-zero walk/sprint multipliers so resting movement resolves to the configured RMC rate while hard zero blockers stay zero. The merge retained the rest lifecycle but dropped this final policy step.
- Decision and behavior: Invoke a narrow RMC post-processor after current modifier subscribers and before equality/dirty checks. It reproduces live RMC's inverse-multiplier calculation, preserves zero-valued blockers, and leaves current refresh suppression while applying network state, dirtying, base-speed ownership, and prediction unchanged.
- Files changed: `Content.Shared/Movement/Systems/MovementSpeedModifierSystem.cs`, `Content.Shared/Movement/Systems/MovementSpeedModifierSystem.RMC.cs`, and `docs/upstream-sync/core-system-audit.md`.
- Validation evidence: Static order review confirms all normal modifier subscribers run first, the RMC override runs once, and the existing replicated update is the sole writer afterward. Targeted Shared DebugOpt `--no-dependencies` build succeeded with 0 errors and 6 pre-existing warnings; exact-path `git diff --check` passed. Tests remain deferred until the 1,000-upstream-commit checkpoint.
- Remaining debt: Runtime/prediction coverage must cross resting with armor, injury, slowdown, sprint/walk inputs, hard movement blockers, standing transitions, pulling, water, and resimulation. The configured value should be balance-checked because it participates in the retained global grounded-walk rule.

## CS-0325 - Restore conditional RMC water contacts

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0324.
- Areas: Movement, Physics contacts, Water, Speed modifiers, Floor occlusion, Prediction, Lifecycle
- Classification: Missing -> Adapted.
- Risk: Medium before the fix because covered RMC water still slowed and visually occluded entities; low-to-medium after it pending dynamic-cover contact coverage.
- Behavior/API delta: Current SS14 applies contact speed modifiers and floor occlusion to every matching physics contact. RMC water prototypes can specify an anchored-cover whitelist; `RMCWaterSystem.CanCollide` suppresses water effects when such a cover occupies the tile. The retained policy remained in use by other water consumers, but the current generic contact and occlusion owners no longer called it.
- Decision and behavior: Add narrow `_RMC14` partial helpers at three current decision points: initial speed-contact registration, modifier recomputation, and initial floor-occlusion registration. Non-water entities return true and keep the unchanged SS14 path; uncovered RMC water retains current physics/event/network ownership, while covered water contributes neither slowdown nor occlusion.
- Files changed: `Content.Shared/Movement/Systems/SpeedModifierContactsSystem.cs`, `Content.Shared/Movement/Systems/SharedFloorOcclusionSystem.cs`, `Content.Shared/_RMC14/Water/SpeedModifierContactsSystem.RMC.cs`, `Content.Shared/_RMC14/Water/SharedFloorOcclusionSystem.RMC.cs`, and `docs/upstream-sync/core-system-audit.md`.
- Validation evidence: Static call-path review confirms the retained whitelist policy gates both contact registration and refresh, gates visual occlusion registration, and is a no-op for ordinary upstream contact modifiers. Targeted Shared DebugOpt `--no-dependencies` build succeeded with 0 errors and 6 pre-existing warnings; exact-path `git diff --check` passed. Tests remain deferred until the 1,000-upstream-commit checkpoint.
- Remaining debt: Current physics start/end events do not re-fire merely because an anchored cover appears or disappears over an existing contact. Runtime work must verify and, if needed, add explicit cover-change recomputation for slowdown and occlusion; it must also cover overlapping water, airborne entities, whitelists, prediction rollback, despawn/shutdown, and contact-set cleanup.

## CS-0326 - Movement subsystem closeout

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0325.
- Scope completed: systems, components, events, prototype contracts, CVars, networking/prediction, authority, tick ordering, lifecycle hooks, and deferred tests across input movers and relays, grounded/weightless motion, speed aggregation and temporary modifiers, standing/resting, climbing, pulling/buckling, mob collision, lag compensation consumers, grid vehicles, and water contact/occlusion.
- Classification: subsystem source audit completed; behavioral parity is not claimed without runtime and prediction tests.
- `Aligned`: Current SS14's shared `VirtualController`/`SharedMoverController`, effective-mover relay, `UpdateCanMoveEvent`, replicated `MovementSpeedModifierComponent`, `RefreshMovementSpeedModifiersEvent`, weightless/off-grid friction, fixed physics contacts, climb DoAfter/cancellation lifecycle, and client/server prediction ownership remain authoritative. Retained `TemporarySpeedModifiersSystem`, stamina/slow callers, RMC standing state, and `SharedRMCLagCompensationSystem` consumers remain connected to current events and CVars.
- `Adapted`: CS-0268, CS-0276 through CS-0280, CS-0282 through CS-0284, CS-0294, CS-0300, CS-0304, and CS-0322 through CS-0325 retain current owners while reconnecting grid-vehicle operator input; pull/fireman and buckle movement lifecycle; rotation/input contexts; rest cancellation; collision mass, overlap, and big-xeno displacement; the distinct RMC detached-mover marker; barricade-aware climb preflight; grounded walk policy; resting speed; and covered-water contact/occlusion policy.
- `Missing`: No additional high-confidence Movement hook with a current content-side mapping remains known after the system/event/component/prototype/CVar sweeps. This is bounded source evidence, not runtime proof.
- `Behavior changed`: The global grounded walk-speed floor is intentional live-RMC balance layered over current speed aggregation. Current effective-mover relay replaces the old `RMCAllowStrapMovement` exception for its three authored wheelchair/power-loader prototypes. Current fixed-step physics, prediction resimulation, weightless movement, climb target-event ownership, pull cleanup, and generated buckle state supersede older implementations while retaining the reviewed RMC policy hooks.
- `Deferred`: Dynamic placement/removal of a water cover during an existing contact may require explicit slowdown/occlusion recomputation. The retained `RMCAllowStrapMovementComponent` is a stale data adapter while the same prototypes install `RelayInputMoverComponent`; remove it only after runtime verification. The older buckle-specific climb preflight, movement/interaction lag-compensation behavior, standing/crawling edge ordering, contact-set transitions, and all runtime matrices remain deferred. `RMCNetCVarsEvent` is also unused on live RMC and is classified stale rather than a merge loss.
- Prototype/data map: RMC human/xeno bases retain RMC input/collision/rest capabilities; xeno base and variants retain collision mass/size policy; wheelchair and power-loader prototypes retain their marker while current systems establish mover relays; water entities retain `RMCWater`, cover whitelists, purifiable/active lifecycle, speed contacts, and floor occlusion. Current movement and RMC prototype contracts compile but still require prototype loading at the checkpoint.
- CVar/network map: current movement pushing/crawling/relative-movement CVars remain owned upstream; replicated/server RMC CVars `rmc.active_input_mover_enabled`, `rmc.movement_pen_cap_subtract`, `rmc.movement_big_xenos_cancel_movement`, `rmc.lag_compensation_milliseconds`, and `rmc.lag_compensation_margin_tiles` remain consumed by their current fork paths. Movement speed, rest state, collision policy data, and mover-relay state remain on shared/networked components; server authority and client prediction use the same current decision owners.
- Upstream betterment retained: cached active/effective-mover state avoids parallel input owners; current shared prediction and server revalidation use one movement calculation; event-based modifier aggregation composes RMC slows/rest with current status effects; fixed physics contact enumeration and capped/buffered mob impulses improve ordering and stability; generated buckle state and relay cleanup make lifecycle transitions explicit.
- Validation evidence: the post-wave targeted Shared DebugOpt `--no-dependencies` build succeeded with 0 errors and 6 pre-existing warnings. Exact-path `git diff --check` ran before every Movement commit. Static review accounts for each retained movement CVar and live RMC hook described above; no test or prototype suite ran at 853/1,000 upstream commits.
- Remaining risk: Runtime matrices remain high-risk for simultaneous input relay changes, prediction rollback, walking/sprinting under stacked modifiers, rest/stand transitions, climb cancellation and buckle state, large/small xeno contacts, lag-compensated abilities, vehicle driving, overlapping water, dynamic covers, map serialization, and component shutdown/despawn.
- Next subsystem: GameTicking and Gamerules, beginning with round-end/start policy, role/loadout spawning, CM arrival/round announcements, and dropship FTL state transitions.

## CS-0327 - Restore RMC round-control policy on current lifecycle hooks

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0326.
- Areas: GameTicking, Round end/restart, Lobby timing, Admin commands, CVars, Lifecycle
- Classification: Missing -> Adapted.
- Risk: High before the fix because `rmc.delay_round_end` and `rmc.lobby_start_paused` remained registered but no longer controlled current round transitions; medium after it pending timer/admin/runtime coverage.
- Behavior/API delta: Live RMC blocks both the in-round end transition and the post-round restart timer while round-end delay is enabled, clears that hold for an explicit `endround`, resets it at restart, pauses a newly prepared lobby when configured, and grants RMC maintainers access to `startround`/`delaystart`. The bulk merge retained the CVars and RMC toggle command but lost those current-owner hooks.
- Decision and behavior: Guard current `RoundEndSystem.EndRound` and its delayed restart callback, clear the hold in the current explicit end command, and use `RoundRestartCleanupEvent` in `RMCGameTickerSystem` for reset/pause policy. Multiple `AdminCommand` attributes are explicitly OR alternatives, so RMC maintainer access is restored without weakening the existing Round permission. Current `GameTicker` run-level transitions, restart cleanup broadcast, timers, lobby countdown/status, scoreboards, and rule authority remain authoritative.
- Files changed: `Content.Server/RoundEnd/RoundEndSystem.cs`, `Content.Server/GameTicking/Commands/EndRoundCommand.cs`, `Content.Server/GameTicking/Commands/StartRoundCommand.cs`, `Content.Server/GameTicking/Commands/DelayStartCommand.cs`, `Content.Server/_RMC14/GameTicking/RMCGameTickerSystem.cs`, and `docs/upstream-sync/core-system-audit.md`.
- Validation evidence: Static call-path review confirms both current `RoundEndSystem` transition points are guarded, explicit `endround` releases the hold, cleanup resets it once per restart, and the current command attribute contract documents alternative permission sets. The final Server build for this combined game wave is recorded at subsystem closeout; exact-path `git diff --check` passed. Tests remain deferred until the 1,000-upstream-commit checkpoint.
- Remaining debt: Runtime coverage must toggle delay before/during the scoreboard timer, invoke automatic and explicit ends, restart with lobby enabled/disabled and zero/non-zero players, exercise paused countdown status, reconnect clients, and verify admin permissions. Current upstream max-time/inactivity rules call `GameTicker.EndRound` directly just as live RMC did and therefore remain outside this hold policy unless a later gameplay decision broadens it.

## CS-0328 - Restore RMC character and loadout spawn contracts

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0327.
- Areas: GameTicking, Character creation, Job spawning, Loadouts, Prototype contracts, Configuration, Lifecycle
- Classification: Missing -> Adapted; inherited-loadout fallback is an upstream-informed predictability improvement.
- Risk: High before the fix because new profiles defaulted to upstream Passenger and retained survivor/synthetic loadout fields had no current spawn owner; medium after it pending profile/prototype/runtime coverage.
- Behavior/API delta: Current SS14 introduced `game.new_character_jobs`, whose default is Passenger; the RMC preset did not override it even though CM uses `CMRifleman`. Ninety-one survivor job declarations retain `useLoadoutOfJob`, and three synthetic loadouts retain `componentsAdd: true`, but the merged current spawning paths ignored both fork fields.
- Decision and behavior: Configure new CM profiles with `CMRifleman`. Resolve `UseLoadoutOfJob` before the current role-loadout/profile/default pipeline, so survivor variants consistently use the base survivor customization and now receive the base default when no saved selection exists instead of conditionally falling back to the variant. At the current `LoadoutPrototype` equip boundary, route component-add loadouts to the retained event/system and skip entity equipment; ordinary current starting gear, inventory, names, specials, identity, and spawn events remain unchanged.
- Files changed: `Resources/ConfigPresets/RMC14/rmc.toml`, `Content.Server/Station/Systems/StationSpawningSystem.cs`, `Content.Shared/Station/SharedStationSpawningSystem.cs`, and `docs/upstream-sync/core-system-audit.md`.
- Validation evidence: Static resource review accounts for all 91 `useLoadoutOfJob` declarations and all three `componentsAdd` synthetic generation choices; `CMRifleman` is an existing job prototype, and profiles/jobs without fork fields retain the current path. Final Shared/Server builds are recorded at subsystem closeout; exact-path `git diff --check` passed. Tests and prototype loading remain deferred until the 1,000-upstream-commit checkpoint.
- Remaining debt: Runtime coverage must create a brand-new CM profile, load old profiles, spawn every base/variant survivor with present and absent saved loadouts, validate entity-spawn-override naming, apply each synthetic generation exactly once, and check reconnect/latejoin. The upstream guest/random job default still names Passenger, and the retained RMC job data has a stale `canBeAntag: false` contract; both need a focused preference/role-policy decision rather than inference here.

## CS-0329 - Restore CM spawn and round announcements

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0328.
- Areas: GameTicking, Player spawn events, Latejoin announcements, ARES/radio, Planet greeting, Round announcements, Prototype filtering
- Classification: Missing -> Adapted; non-CM coexistence is intentionally Behavior changed from live RMC.
- Risk: Medium before the fix because RMC arrival routing and the selected-planet greeting were disconnected and CM rounds could select upstream announcement text; low-to-medium after it pending spawn/radio/runtime coverage.
- Behavior/API delta: Live RMC suppressed the vanilla arrival path globally, called `MarinePresenceAnnounceSystem` directly after spawn, sent the selected planet greeting, and filtered round-announcement prototypes through CM identity. Current SS14 now broadcasts a complete `PlayerSpawnCompleteEvent`; the merge retained the RMC announcement implementation and CM prototype marker but no producer connection.
- Decision and behavior: Subscribe the RMC system to the current completed-spawn event, require a CM job, then reuse the retained rank/squad/department/head-aware ARES/radio path and send the selected planet to that player. Suppress the vanilla arrival announcement only for CM jobs, allowing non-CM content to retain current SS14 behavior, and enumerate round announcements through `EnumerateCM`. Current spawn completion ordering, job assignment/logging, station greeting, event broadcast, and silent/latejoin gates remain authoritative.
- Files changed: `Content.Server/GameTicking/GameTicker.Spawning.cs`, `Content.Server/GameTicking/GameTicker.RoundFlow.cs`, `Content.Server/_RMC14/Announce/MarinePresenceAnnounceSystem.cs`, `Resources/Locale/en-US/_RMC14/job/greetings.ftl`, and `docs/upstream-sync/core-system-audit.md`.
- Validation evidence: Static flow review finds one completed-spawn consumer for CM arrivals, one vanilla path for non-CM jobs, preserved `lateJoin`/`silent` checks in the retained announcer, and CM-aware round-announcement enumeration. Final Server build is recorded at subsystem closeout; exact-path `git diff --check` passed. Tests and localization/prototype loading remain deferred until the 1,000-upstream-commit checkpoint.
- Remaining debt: Runtime coverage must exercise round-start, latejoin, silent/admin spawn, every department/head/squad fallback, custom arrival sounds, missing ARES/station/planet cases, and CM-filter modes. The event exposes the job prototype rather than the mind-derived display name, so the announcement currently localizes `job.Name`; verify jobs whose runtime role name differs. Broad lobby/station branding remains outside this behavioral hook.

## CS-0330 - Reconcile RMC dropships with current FTL state and events

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0329.
- Areas: GameTicking, Dropships, Shuttle FTL, Networking/prediction, UI, Equipment lifecycle, Fire cleanup, Timing
- Classification: Missing/Behavior changed -> Adapted; two transition-order differences remain Deferred.
- Risk: Critical before the fix because the retained networked dropship state was never synchronized, shared utility/weapon checks read transient `FTLComponent` state that current clients do not receive, and retained custom before-FTL events had no raiser; medium after it pending full flight/runtime coverage.
- Behavior/API delta: Current SS14 creates/removes `FTLComponent` for each trip and exposes `FTLRequestEvent`, `FTLStartedEvent` with the source map/matrix, and `FTLCompletedEvent`; live RMC patched extra pre-start/pre-finish events and explicit state updates into that owner. The merge removed those raisers while leaving mounted-equipment subscriptions, death-floor/fire hooks, landing presentation, and a networked `DropshipComponent.State` contract behind.
- Decision and behavior: Make the RMC networked state default to Available and synchronize it at request, start, completion, every observed Arriving transition, and component removal. Shared predicted utility, weapon, paradrop, ordnance, and power-loader checks now consume that state, while server flight requests/cancellation and destination validation retain current `FTLComponent` authority. Relay current `FTLStartedEvent` to mounted equipment; reconstruct the departed grid's tile centers from `FromMapUid`/`FTLFrom` to spawn death-floor entities without a pre-event; clear destination tile fire on current completion. The existing alarm, UI, destination, landing-light, arrival-sound, crash, and recharge paths receive each state update.
- Files changed: `Content.Server/_RMC14/Dropship/DropshipSystem.cs`, `Content.Server/_RMC14/Dropship/Utility/RMCEquipmentDeployerSystem.cs`, `Content.Server/_RMC14/Shuttles/RMCShuttleSystem.cs`, `Content.Shared/_RMC14/Dropship/DropshipComponent.cs`, `Content.Shared/_RMC14/Dropship/SharedDropshipSystem.cs`, `Content.Shared/_RMC14/Dropship/Utility/Systems/DropshipUtilitySystem.cs`, `Content.Shared/_RMC14/Dropship/Weapon/SharedDropshipWeaponSystem.cs`, and `docs/upstream-sync/core-system-audit.md`.
- Validation evidence: Static event/state review accounts for request, Starting, Travelling, Arriving, Cooldown, removal/Available, mounted start/completion relays, all shared in-flight consumers, and both retained shuttle side effects. Stale `BeforeFTLStartedEvent`/`BeforeFTLFinishedEvent` references are zero. Final Shared/Server builds are recorded at subsystem closeout; exact-path `git diff --check` passed. Tests remain deferred until the 1,000-upstream-commit checkpoint.
- Remaining debt: Arriving and removal are detected by the bounded dropship update loop and can lag the current shuttle transition by at most one system tick. Equipment undeploy now occurs immediately after the grid enters FTL rather than immediately before movement; tile-fire cleanup now occurs after arrival/Smimsh rather than before placement. Runtime evidence is required before introducing a broader upstream-owner hook. Live RMC's global shuttle startup/arrival audio replacement and `AreaComponent` Smimsh exclusion were not restored; current dropship destination audio remains, and area entities are currently created in nullspace, so both are explicit policy/runtime decisions rather than assumed losses.

## CS-0331 - Preserve RMC replay planet-map metadata

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0330.
- Areas: GameTicking, Gamerules, Replays, Operational metadata, Map lifecycle
- Classification: Missing -> Adapted.
- Risk: Medium before the fix because replay consumers could no longer identify the selected distress-signal planet; low after the static fix pending replay-schema/runtime coverage.
- Behavior/API delta: Current SS14 writes one singular `map` key for its selected station map. Live RMC replaced that field with a `maps` list containing both the ship/station map and selected planet. The bulk merge adopted the current replay writer and silently lost the planet entry used by RMC-aware replay tooling.
- Decision and behavior: Retain current SS14's singular `map` key and add the RMC `maps` list through a narrow `_RMC14` partial. This is backward-compatible for current consumers while restoring the live-RMC list contract and planet context; the current replay stop event, serialization manager, round-end fields, write task, and file lifecycle remain authoritative.
- Files changed: `Content.Server/GameTicking/GameTicker.Replays.cs`, `Content.Server/_RMC14/GameTicking/GameTicker.Replays.RMC.cs`, and `docs/upstream-sync/core-system-audit.md`.
- Validation evidence: Static flow review confirms the metadata is assembled on the replay-stop path, omits unavailable map values, preserves current `map`, and serializes live RMC's ordered ship/planet list under `maps`. The targeted Server build and exact-path `git diff --check` are recorded by this commit. Tests and replay recording remain deferred until the 1,000-upstream-commit checkpoint.
- Remaining debt: Live RMC also emitted `planet_map` from the server status callback by resolving the distress-rule system on that callback's non-simulation thread. Current SS14 explicitly protects this path as cross-thread status data, so direct ECS access would regress predictability and thread safety. Restore that operational field only through a main-thread-updated immutable/cache snapshot, then validate concurrent requests, round cleanup, map reselection, lobby state, replay parsers, and old/new schema consumers.

## CS-0332 - Restore lesser-drone ghost-role slot lifecycle

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0331.
- Areas: Gamerules, Ghost roles, Xeno spawning, Slot accounting, Lifecycle, EUI
- Classification: Missing -> Adapted.
- Risk: High before the fix because a full lesser-drone spawner stayed advertised but untakeable and never reopened after a drone died; low-to-medium after the source fix pending takeover/death/runtime coverage.
- Behavior/API delta: Current `GhostRoleSystem` increments `CurrentTakeovers` and marks a non-deleting spawner's role `Taken` when its capacity is reached. Live RMC's dynamic spawner setters then registered or unregistered the role whenever its current or available count changed. The bulk merge adapted those setters to current component fields and EUI refreshes but dropped the `Taken` transition and role registry lifecycle.
- Decision and behavior: Reconcile `Taken` after every pylon-driven current/available change, unregister full/zero-capacity roles, and register them again when a slot becomes available. The existing RMC pylon update remains authoritative for living-xeno ratios, ovipositor cooldowns, live lesser-drone tracking, death cleanup, and capacity calculation; current `GhostRoleSystem` remains authoritative for takeover validation, entity spawning, mind transfer, registry identifiers, raffles, and EUI publication.
- Files changed: `Content.Server/Ghost/Roles/GhostRoleSystem.cs`, `Content.Server/_RMC14/Ghost/Roles/GhostRoleSystem.Spawner.RMC.cs`, `Content.Server/_RMC14/Xenonids/Construction/XenoPylonSystem.cs`, and `docs/upstream-sync/core-system-audit.md`.
- Validation evidence: Static lifecycle review follows initialization, first availability, successful takeover, full-capacity removal, live-drone death, count decrement, and role re-registration. Both count setters now use the same registry transition and EUI update. The targeted Server build and exact-path `git diff --check` are recorded by this commit. Tests remain deferred until the 1,000-upstream-commit checkpoint.
- Remaining debt: Runtime coverage must exercise zero/one/multiple slots, simultaneous applicants, raffle and first-come paths, ovipositor/no-ovipositor cooldowns, hive extinction/recovery, drone death/deletion/component removal, pylon deletion, round cleanup, duplicate registration protection, EUI counts, and job bans/delays still marked TODO by live RMC.

## CS-0333 - Restore RMC content service registrations

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0332.
- Areas: GameTicking, Gamerules, Client/server bootstrap, IoC lifecycle, Networking, Persistence
- Classification: Missing -> Adapted.
- Risk: High before the fix because retained systems injected unregistered RMC managers and could fail during client/server dependency-graph construction despite successful assembly compilation; medium after the static/build fix pending full process startup.
- Behavior/API delta: Live RMC manually registered four client and seven server singleton services during content IoC setup. The bulk merge adopted current SS14's `IDependencyCollection` entry points but dropped all RMC registrations while leaving their consumers, message handlers, persistence hooks, and `IPostInjectInit` work intact.
- Decision and behavior: Register client `LinkAccountManager`, `RMCPlayTimeManager`, `CommendationsManager`, and `TacticalMapSettingsManager`, plus server `LinkAccountManager`, `RMCPlayTimeManager`, `RMCDiscordManager`, `MentorManager`, `CommendationManager`, `RMCActionsManager`, and `RMCChatBansManager`, through the current dependency collection. Current graph construction preserves one instance per concrete registration, injects its dependencies, and invokes post-injection initialization; no parallel service owner is introduced.
- Files changed: `Content.Client/IoC/ClientContentIoC.cs`, `Content.Server/IoC/ServerContentIoC.cs`, and `docs/upstream-sync/core-system-audit.md`.
- Validation evidence: Static consumer/registration review accounts for all eleven live-RMC services and found no duplicate same-side network-message registration. Targeted Client/Server builds and exact-path `git diff --check` are recorded by this commit. Tests and executable startup remain deferred until the 1,000-upstream-commit checkpoint.
- Remaining debt: Start both processes and validate dependency graph construction, connect/disconnect message registration, account/playtime/commendation/mentor/action/chat-ban flows, and persistence reload. `RMCDiscordManager` retains live RMC's eager post-injection background work without an explicit shutdown hook; `TacticalMapSettingsManager` retains legacy constructor-time global IoC fallback behavior, though normal startup registration now prevents fragmented settings instances.

## CS-0334 - Restore RMC ghost-role round-time gates

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0333.
- Areas: Gamerules, Ghost roles, Raffles, Round timing, Prototype contracts, EUI
- Classification: Missing -> Adapted.
- Risk: High before the fix because retained raffle data authored an unknown `roundTimeRequirement` field and its retained RMC timing system had no event producer; medium after the source/build fix pending prototype and clock-boundary coverage.
- Behavior/API delta: Current SS14 copies prototype raffle durations into a server-side raffle component at `ComponentInit` and then owns countdown updates, participation, winner selection, and EUI timing. Live RMC extended initialization with a round-age gate. The bulk merge retained the 900-second RMS requirement and consumer but dropped both its schema field and initialization event.
- Decision and behavior: Restore the optional field through an `_RMC14` settings partial and raise the retained RMC event after current settings are copied. Its server consumer adds `max(0, requirement - current round duration)` to the configured countdown. Ordinary SS14 raffles default to zero and retain unchanged timing; current server authority, pause handling, registry, winner selection, mind transfer, and UI publication remain authoritative.
- Files changed: `Content.Server/Ghost/Roles/GhostRoleSystem.cs`, `Content.Server/_RMC14/Ghost/Roles/GhostRoleSystem.RMC.cs`, `Content.Shared/_RMC14/Ghost/GhostRoleRaffleSettings.RMC.cs`, and `docs/upstream-sync/core-system-audit.md`.
- Validation evidence: Static flow review connects the sole retained `roundTimeRequirement: 900` declaration to one initialization producer and one RMC consumer, with the adjustment applied once before normal countdown updates. Targeted Shared and Server builds succeeded with 0 errors and 6/4 known warnings; exact-path `git diff --check` passed. Tests and prototype loading remain deferred until the 1,000-upstream-commit checkpoint.
- Remaining debt: Runtime coverage must start the raffle before/at/after 900 seconds, exercise quick-lottery mode, pause/unpause, late participants, cumulative/max duration, cancellation, role deletion, round restart, EUI end times, and winner transfer. The retained formula intentionally adds the configured countdown after the round-age gate rather than replacing it.

## CS-0335 - Restore nested-victim ghost return

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0334.
- Areas: Gamerules, Ghosting, Xeno nests, Mind authority, Networking, Lifecycle
- Classification: Missing -> Adapted.
- Risk: High before the fix because freed nested victims could receive a return button whose request never transferred their mind back; low-to-medium after the source/build fix pending disconnect/deletion/runtime coverage.
- Behavior/API delta: RMC nesting moves an eligible victim's mind into a ghost, records the original body in `RMCGhostReturnComponent`, and enables the standard return action when the victim is freed. Current `GhostSystem` validates that network request and normally invokes visiting-mind return, but this RMC ghost owns the mind rather than visiting, so the merged path did nothing after losing its marker consumer.
- Decision and behavior: At the current validated server request boundary, consume the server-owned return target and use current `MindSystem.TransferTo`; if the marker, mind, or live target is unavailable, retain upstream `UnVisit` fallback. The client supplies no destination, so current actor/ghost/`CanReturnToBody` validation and server authority remain intact. Target termination is rejected rather than reproducing live RMC's stale-transfer attempt.
...
## CS-0338 - Restore CM role preference and latejoin filtering

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0337.
- Areas: Gamerules, Role preferences, Latejoin, Prototype filtering, Client UI
- Classification: Missing -> Adapted.
- Risk: High before the fix because the CM profile editor exposed upstream departments plus hidden CM jobs and could persist preferences that the distress-signal station cannot assign; low-to-medium after the source/build fix pending UI/prototype matrices.
- Behavior/API delta: Current SS14 enumerates every department, suppresses `EditorHidden`, and includes jobs with `SetPreference`. RMC adds CM prototype identity, a separate gameplay `Hidden` policy on departments/jobs, and optional department `CustomName`; live latejoin also enumerates only CM departments. Those fields and the generic CM filter survived, but both current UI owners bypassed them after the merge.
- Decision and behavior: Enumerate departments through `EnumerateCM`, preserve current `EditorHidden` and `SetPreference` gates, restore RMC `Hidden` exclusion, and use authored `CustomName` before the current localized fallback. Apply the same CM-only department boundary to latejoin. Current station job availability, role requirements, whitelist/timer checks, priority controls, and job request networking remain authoritative.
- Files changed: `Content.Client/Lobby/UI/HumanoidProfileEditor.Roles.cs`, `Content.Client/LateJoin/LateJoinGui.cs`, and `docs/upstream-sync/core-system-audit.md`.
- Validation evidence: Static prototype review confirms hidden jobs such as `CMOrdnanceTech`, `CMResearcher`, `RMCFieldDoctor`, and `CMRiotControl` would otherwise pass inherited `SetPreference`; current `CMXeno` consumes `CustomName`. The targeted Client build and exact-path `git diff --check` are recorded by this commit. Tests and UI/prototype loading remain deferred until the 1,000-upstream-commit checkpoint.
- Remaining debt: Runtime coverage must open profile and latejoin UI against CM, mixed, and dev stations; inspect empty/hidden/custom departments; save/reload priorities; enforce requirements/whitelists; and handle prototype reload. Live RMC's armor, squad, per-job rank controls, dummy-gear preview, and CM-filtered antagonist preferences remain separate UI debt.

## CS-0339 - Restore shared RMC playtime tracker contracts

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0338.
- Areas: Gamerules, Job requirements, Playtime, Prototype lifecycle, Client presentation
- Classification: Missing -> Adapted.
- Risk: High before the fix because current initialization inserted shared tracker IDs into a one-to-one dictionary and could throw before gameplay; medium after the source/build fix pending prototype/startup and UI coverage.
- Behavior/API delta: Current SS14 assumes one job per `PlayTimeTrackerPrototype`. Retained RMC data has 333 explicit job-tracker declarations, with ten tracker IDs shared across 73 jobs; `RMCJobSurvivorMilitary` alone maps fourteen jobs. Nine grouped survivor trackers also retain authored `name` fields, while the merge dropped `Name`, `ShowInStatsMenu`, grouped statistics, and grouped requirement labels.
- Decision and behavior: Build a typed tracker-to-job-list map in ordinal job-ID order, expose its read-only list, and retain the old single-job API as a deterministic first-job wrapper. Restore the two RMC tracker fields, one statistics row per tracker, authored names or complete localized job-name fallback, and highest-weight represented department color in requirement errors. Current prototype-reload setup, playtime dictionary authority, requirement inversion, localization formatting, and client display pipeline remain authoritative.
- Files changed: `Content.Shared/Roles/Jobs/SharedJobSystem.cs`, `Content.Shared/Roles/JobRequirement/RoleTimeRequirement.cs`, `Content.Shared/_RMC14/Marines/Roles/PlayTimeTrackerPrototype.cs`, `Content.Client/Players/PlayTimeTracking/JobRequirementsManager.cs`, and `docs/upstream-sync/core-system-audit.md`.
- Validation evidence: Static resource accounting finds ten duplicate tracker groups and confirms every duplicate is retained in the list map; the prior dictionary `Add` failure path is gone. Targeted Shared and Client builds succeeded with 0 errors and 6/8 known warnings; exact-path `git diff --check` passed. Tests and prototype/startup UI coverage remain deferred until the 1,000-upstream-commit checkpoint.
- Remaining debt: Runtime coverage must initialize and hot-reload all job prototypes, render named/unnamed grouped rows, inspect department color/timer messages, update playtime, and exercise inverted requirements. `showInStatsMenu: false` has no current author. The compatibility stats consumer still has two `basePlaytimeTracker` jobs for `RMCJobSurvivorMilitary` and none for `CMJobCorrespondent`, an existing live-RMC data ambiguity.

## CS-0340 - Restore RMC role-timer exclusions

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0339.
- Areas: Gamerules, Job assignment, Playtime requirements, Persistence, Client/server authority
- Classification: Missing -> Adapted.
- Risk: High before the fix because admins could persist an exclusion through the retained command/manager while all current assignment and UI paths still rejected the player; medium after the source/build fix pending load-order and assignment matrices.
- Behavior/API delta: Current SS14 enforces playtime requirements in direct server checks, disallowed-job reporting, round-start candidate filtering, and client presentation. Live RMC can exclude particular users from selected job timers. Its service, command, database hooks, messages, and state survived, but the merge removed every decision-owner call.
- Decision and behavior: Consult the retained server manager in all three current authority paths and remove/bypass only excluded job timer failures. On the client, evaluate bans and whitelist first, then bypass role-time requirements for synchronized exclusions and relay manager updates to the current UI refresh event. Current server database state remains authoritative; exclusions do not bypass bans, whitelists, antag rules, or non-time job restrictions.
- Files changed: `Content.Server/Players/PlayTimeTracking/PlayTimeTrackingSystem.cs`, `Content.Client/Players/PlayTimeTracking/JobRequirementsManager.cs`, and `docs/upstream-sync/core-system-audit.md`.
- Validation evidence: Static call-path review connects direct latejoin checks, disallowed-role publication, round-start list pruning, client requirement presentation, and exclusion update propagation. The implementation deliberately retains current SS14's correct unmet-requirement predicate rather than live RMC's apparently inverted `GetDisallowedJobs` condition. Targeted Client/Server builds and exact-path `git diff --check` are recorded by this commit. Tests remain deferred until the 1,000-upstream-commit checkpoint.
- Remaining debt: Runtime coverage must add/remove exclusions online and offline, load before/after preference UI opens, assign at round start and latejoin, reconnect as another account, combine bans/whitelists/multiple timer requirements, and observe DB failure. Client exclusion state is not cleared on disconnect and can briefly display the previous session until the next server message; server authority prevents access escalation.

## CS-0341 - Restore inherited RMC job whitelists

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0340.
- Areas: Gamerules, Job assignment, Whitelists, Client/server authority, Prototype contracts
- Classification: Missing -> Adapted.
- Risk: High before the fix because two retained leadership/survivor jobs declared a parent whitelist that neither server assignment nor client presentation honored; low-to-medium after the source/build fix pending database/UI matrices.
- Behavior/API delta: Current SS14 checks only a job's direct whitelist ID. RMC adds `JobPrototype.WhitelistParent`, authored by the survivor command officer and Solaris field-operations leader to inherit `CMCommandingOfficer` access. The field survived the bulk merge, but both decision owners reverted to direct membership.
- Decision and behavior: Follow the job/parent chain on both server and client after the current whitelist CVar and initial `Whitelisted` gate. Any membership in that chain grants the job; missing parents and cycles deny with a diagnostic. This retains current server database authority and produces identical client presentation, while adding cycle protection absent from live RMC and avoiding its server/client mismatch when a parent prototype is not itself marked whitelisted.
- Files changed: `Content.Server/Players/JobWhitelist/JobWhitelistManager.cs`, `Content.Server/_RMC14/JobWhitelists/JobWhitelistManager.RMC.cs`, `Content.Client/Players/PlayTimeTracking/JobRequirementsManager.cs`, `Content.Client/_RMC14/JobWhitelists/JobRequirementsManager.Whitelist.RMC.cs`, and `docs/upstream-sync/core-system-audit.md`.
- Validation evidence: Static resource review accounts for both `whitelistParent` declarations and confirms both chains terminate at `CMCommandingOfficer`. Targeted Client/Server builds and exact-path `git diff --check` are recorded by this commit. Tests and database/prototype loading remain deferred until the 1,000-upstream-commit checkpoint.
- Remaining debt: Runtime coverage must test direct/parent/no membership, disabled CVar, data not yet loaded, add/remove while connected, malformed/missing/cyclic parents, preference and latejoin refresh, round-start assignment, and reconnect. Decide separately whether descendants of a non-whitelisted parent should ever inherit implicit access; this adaptation intentionally requires actual membership in the chain.

## CS-0342 - Complete the RMC overflow fallback contract

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0341.
- Areas: Gamerules, Character profiles, Job assignment, Client preview, Prototype contracts
- Classification: Behavior changed -> Adapted.
- Risk: High before the fix because legacy/empty profiles and fallback UI still resolved to upstream Passenger, which is not a distress-signal station job; low-to-medium after the source/build fix pending old-profile and mixed-preset coverage.
- Behavior/API delta: CS-0328 configured newly created RMC profiles with `CMRifleman`, but current `SharedGameTicker` retained SS14's static Passenger fallback. That value still initializes empty profiles, drives overflow labels/previews, and backs `GameTicker` assertions, leaving the earlier adaptation incomplete.
- Decision and behavior: Restore live RMC's `CMRifleman` fallback and rifleman localization key at the existing shared contract. This keeps profile defaults, preview, labels, and server validation consistent with the RMC preset. Current configured `game.new_character_jobs`, profile persistence, station job availability, and assignment authority remain unchanged.
- Files changed: `Content.Shared/GameTicking/SharedGameTicker.cs` and `docs/upstream-sync/core-system-audit.md`.
- Validation evidence: Static consumer review accounts for profile initialization, profile preview, overflow label, and server fallback assertions; `CMRifleman` and `cm-job-name-rifleman` are retained prototypes/locale. Targeted Shared, Client, and Server builds plus exact-path `git diff --check` are recorded by this commit. Tests and profile/prototype loading remain deferred until the 1,000-upstream-commit checkpoint.
- Remaining debt: Runtime coverage must create empty/new/legacy profiles, preview and save them, join full/overflow rounds, remove a selected job, and verify fallback assignment. If CMU later supports SS14 and RMC presets in one binary, replace this global static fallback with a preset/config-owned value rather than reverting to Passenger.

## CS-0343 - Restore the RMC spawn-as-job admin verb

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0342.
- Areas: Gamerules, Admin verbs, Job spawning, Dialog networking, Prototype filtering
- Classification: Missing -> Adapted.
- Risk: High before the fix because the retained authoritative handler, dialog event, localization, and spawn-menu names had no verb producer, making the operation unreachable; low-to-medium after the source/build fix pending admin/runtime matrices.
- Behavior/API delta: Live RMC patched `AdminVerbSystem` with a player-targeted "spawn here as job" menu. Current SS14 reorganized admin verbs and the merge dropped that producer while retaining `RMCAdminSystem`'s event consumer. Hundreds of RMC job prototypes still provide the display contract.
- Decision and behavior: Add a separate `_RMC14` server verb subscriber instead of broad upstream-file edits. Require an admin actor and player-controlled target, revalidate both when activated, enumerate CM jobs, use `SpawnMenuRoleName` or localized job name, sort ordinally, and open the current RMC dialog with the retained event payload. `RMCAdminSystem` remains authoritative for final entity validation, spawn position/entity, mind transfer, role assignment, spawn-complete event, logging, and cleanup.
- Files changed: `Content.Server/_RMC14/Admin/RMCAdminSpawnJobVerbSystem.cs` and `docs/upstream-sync/core-system-audit.md`.
- Validation evidence: Static producer/consumer review finds one current event producer and the retained authoritative consumer, with CM prototype filtering and activation-time permission revalidation. The targeted Server build and exact-path `git diff --check` are recorded by this commit. Tests and dialog/prototype loading remain deferred until the 1,000-upstream-commit checkpoint.
- Remaining debt: Runtime coverage must open/cancel/select as admin and non-admin, target connected/disconnected players, delete/replace the target between menu and choice, inspect every spawn-menu label, verify mind/role cleanup, and audit logs. Live RMC's adjacent random-name and long-string subtle-message admin adaptations remain Deferred.

## CS-0344 - Restore the RMC lobby Join Xeno entry point

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0343.
- Areas: Gamerules, Lobby, Xeno spawning, Networking, Client lifecycle
- Classification: Missing -> Adapted.
- Risk: High before the fix because the retained direct-larva request/status pipeline had no lobby entry point and observing first applies a different queue/death policy; low-to-medium after the source/build fix pending state/network matrices.
- Behavior/API delta: Current SS14's split lobby removed RMC's button and `LobbyState` handler while `RMCLobbyUIController`, `JoinXenoSystem`, window, locale, requests, and server validation survived. The existing public open method had no caller. Live RMC also requested status from every response-driven refresh, creating a request/reply loop.
- Decision and behavior: Add the localized hidden button to current `LobbyGui` and let the fork controller own state-enter/exit subscriptions. Show it only when the server-synchronized ticker reports a started round, pair its styling with Ready, close it on pre-round/restart/exit, and guard direct opens outside an active lobby/round. Render the cached count, request authoritative status once per open, and leave join validation/spawning to the existing shared/server system. This retains current lobby ownership while removing the live feedback loop.
- Files changed: `Content.Client/Lobby/UI/LobbyGui.xaml`, `Content.Client/_RMC14/Lobby/RMCLobbyUIController.cs`, and `docs/upstream-sync/core-system-audit.md`.
- Validation evidence: Static lifecycle review connects ticker status, visibility, named handler attach/detach, one status request, server response, open-window refresh, join request, and server rejection for joined/no-hive sessions. The targeted Client build and exact-path `git diff --check` are recorded by this commit. Tests and executable UI/network coverage remain deferred until the 1,000-upstream-commit checkpoint.
- Remaining debt: Runtime coverage must enter/leave lobby before/during/after rounds, reconnect, open repeatedly, receive zero/nonzero larva updates, click during races, exhaust larvae, join/no-hive paths, resize/styles, and restart cleanup. `IsRoundStarted` is a coarse non-pre-round value and remains true through post-round until restart; exact post-round suppression requires a richer authoritative phase contract.

## CS-0345 - GameTicking and Gamerules subsystem closeout

- Upstreams compared: live SS14 `fbb3c79b2d206eede2210fbbf5ca1c237c262767`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, and CMU through CS-0344.
- Scope completed: systems, components, events, presets/prototypes, CVars/configuration, networking/EUIs, client presentation, server authority, timing, lifecycle hooks, operational metadata, and deferred tests across run levels; lobby/start/end/restart; rule selection; Distress Signal map/planet/vote flow; job slots/trackers/requirements/whitelists/loadouts/preferences; round-start/latejoin/xeno/ghost spawning; announcements; survivor/xeno balance; objectives/win conditions; scuttle/hijack/dropships; round results; replay/status metadata; cleanup; and admin round/spawn operations.
- Classification: subsystem source audit completed; behavioral parity is not claimed without executable startup, prototype loading, runtime, networking, and timing tests.
- `Aligned`: Current SS14's `GameRuleSystem` lifecycle, rule-state events, preset selection, station-job authority, job/antag bans, `IsRoleAllowedEvent`, mind-role containers, `RulePlayerSpawningEvent`, completed player-spawn event, round-restart cleanup, current status synchronization, replay writer, and server-validated ghost/admin/dialog paths remain the state owners. The retained Distress Signal preset/rule graph, player scaling, planet/map selection, survivor/xeno spawning, queen boost, power/ARES, scuttle, hijack, win conditions, and round-end flow remain represented through current APIs; no additional high-confidence server gameplay hook was found after the final gap sweep.
- `Adapted`: CS-0327 through CS-0344 restore round-end hold/lobby pause/maintainer access; CM character/loadout defaults and component loadouts; CM arrivals/planet greeting/round announcement filtering; dropship FTL state/events; replay ship/planet metadata; dynamic lesser-drone slot lifecycle; content IoC registrations; raffle round-time gates and nested-victim return; authored greetings and RMC admin observer; CM role filtering; one-to-many playtime trackers/grouping; timer exclusions; inherited whitelists; rifleman overflow fallback; spawn-as-job admin verb; and direct lobby Join Xeno. Each keeps current lifecycle/authority and places fork policy in narrow `_RMC14` seams where available.
- `Missing`: The split current profile editor still has no armor, squad, per-job rank, playtime-perk, or xeno name-prefix/postfix controls although their stored fields and server consumers remain. It also omits `DummyStartingGear`/armor-variant preview behavior and CM-only antagonist filtering. RMC playtime-statistics and commendation windows are registered but have no current entry point. These are confirmed client behavior losses, not compilation issues, and require a separate current-UI adaptation wave.
- `Behavior changed`: CM jobs suppress vanilla arrival announcements while non-CM jobs intentionally retain current SS14 behavior. The playtime disallowed-job path intentionally retains current SS14's correct unmet-requirement predicate instead of live RMC's inverted-looking branch. Current FTL start/completion ordering, fixed run-level events, mind-role containers, profile/loadout defaults, and status-thread ownership supersede old implementations. Generic lobby branding/planet information and `golobby` permission differ from live RMC and remain policy choices rather than assumed CMU text/security policy.
- `Deferred`: Restore `planet_map` status data only through a main-thread-updated immutable/cache snapshot; direct live-RMC ECS access from the status thread is rejected. Dropship Arriving/removal can lag by one tick, equipment/fire hooks run on current post-transition events, and old global shuttle audio/area Smimsh policy remain runtime decisions. The larger profile/stats/commendation UI wave, generic branding/planet text, first-15-minute observe warning, random-name/long-string admin verbs, stale `canBeAntag: false`, `golobby` permission, Join Xeno post-round visibility, mind-role PVS, xeno crit ghosting, and all runtime/prototype/network matrices remain explicitly deferred.
- Intentional CM/RMC divergence retained: `CMDistressSignal` default/fallback preset; `CMRifleman` character/overflow fallback; RMC job-slot scaling, survivor/xeno ratios and minimums, queen boost, map/planet voting and operation naming, preflight/daylight/miasma policy, round-end hold and lobby pause, CM filtering, RMC loadout inheritance/component generation, rank/squad/ARES arrivals, round announcements, burrowed-larva joins, ghost raffle gates, admin ghost/spawn tools, dropship flight/equipment, scuttle/hijack, and RMC round results.
- Prototype/config map: `cm_game_presets.yml` retains `CMDistressSignal`; RMC job/department/loadout/raffle/dropship/admin-observer prototypes are connected to the owners above; `rmc.toml` sets `game.new_character_jobs = "CMRifleman"`; the shared fallback now agrees. All 91 `useLoadoutOfJob` declarations, three component-add synthetic loadouts, two `whitelistParent` declarations, ten duplicate tracker groups across 73 jobs, nine named grouped trackers, the RMS 900-second raffle gate, and both hive-core lesser-drone spawners are accounted for. Prototype deserialization is not proven until the checkpoint suite runs.
- CVar/network/timing map: retained round/map/balance CVars remain consumed by Distress Signal, including round-end delay, lobby pause, survivor ratios, xeno minimums, queen boost, map voting, LZ miasma, preflight, sunrise/sunset, and hijack policy. Current lobby status events, job-whitelist/playtime messages, ghost-role EUI/raffle timing, Join Xeno status/request events, player-spawn completion, networked dropship state, FTL request/start/completion, and server-side round timers remain authoritative. Client UI predicts no assignment outcome; server/database/rule systems revalidate every material join, role, whitelist, timer, ghost, and admin action.
- Upstream betterment retained: current rule/job restriction events compose without parallel assignment owners; completed-spawn events give announcements a stable lifecycle point; current profile/loadout fallback is deterministic; typed one-to-many tracker maps and ordinal ordering remove startup ambiguity; current mind-role containers and server revalidation make role authority explicit; current FTL events plus replicated dropship state replace transient client-only component reads; and current cross-thread status isolation avoids live RMC's unsafe entity-system lookup.
- Validation evidence: final targeted DebugOpt `--no-restore --no-dependencies` builds succeeded for Shared with 0 errors/6 known warnings, Client with 0 errors/8 known warnings, and Server with 0 errors/4 known warnings. Exact-path `git diff --check` ran before every GameTicking/Gamerules commit. Static source/resource review accounts for every finding above, but no executable startup, prototype linter, test suite, or gameplay matrix ran at 853/1,000 upstream commits.
- Remaining risk: High-risk runtime matrices remain for round start/end/restart timers; IoC startup; all preset/prototype deserialization; profile migration; job/loadout/whitelist/timer assignment; survivor/xeno/ghost/latejoin concurrency; raffle and nested return; announcements/radio; dropship state/timing; scuttle/hijack/win conditions; admin verbs; reconnect/cleanup; status/replay consumers; and client/server resimulation or message races.
- Next recommended wave: reach the 1,000-upstream-commit checkpoint and run the queued prototype, targeted integration, and full build validation before implementing the deferred Gamerules profile/stats/commendation UI wave.

## CS-0346 - Detach spawned entities from vehicle transform trees

- Upstream: [space-wizards/space-station-14#44764](https://github.com/space-wizards/space-station-14/pull/44764), `887e80fd08a196228765b5b70c72b80d800203f6`, 2026-07-20.
- Areas: Shooting, Interactions, Physics, Prediction, Transform lifecycle
- Classification: Missing -> Ported.
- Risk: High before the fix because projectiles, casings, produce, eggs, and use-spawned items could inherit a moving vehicle/user entity as transform parent and continue moving with it; medium after the source/build fix pending vehicle/prediction matrices.
- Behavior/API delta: Generic `Spawn` preserves the parent encoded by `EntityCoordinates`. When firing or producing content from an entity on/in a vehicle, temporary ammo and ordinary products could remain children of that moving entity instead of entering the surrounding grid/map transform tree. Cartridge ejection likewise changed coordinates without explicitly attaching to the grid/map. The new upstream head uses current position-aware spawn/drop APIs at each proven boundary.
- RMC/CMU divergence: RMC moving-gun coordinate resolution, remote/attachable gun ownership, ammo-provider events, projectile authority, recoil/ejection impulse, prediction, hands, hunger, botany, and stack-merge behavior remain unchanged. The adaptation only changes the initial transform parent and uses the current hand pickup API after an item is already placed on the floor.
- Decision and behavior: Use `SpawnAtPosition` for basic, battery, revolver, and solution ammo plus botany seed/products; use `SpawnNextToOrDrop` for eggs, sericulture, and use-spawned items; use `TryPickupAnyHand` after floor placement; and atomically set casing coordinates/rotation before `AttachToGridOrMap`. These current APIs preserve world position while preventing accidental vehicle/entity parenting and make client/server shared gun spawning take the same transform path.
...
## CS-0348 - Restore ordinary gun projectile prediction

- Upstreams compared: live SS14 `887e80fd08a196228765b5b70c72b80d800203f6`, live RMC `b6d677947dd8ebcb06194a66798938645fed5a54`, the pre-rebase CMU prediction path, and CMU through CS-0347.
- Areas: Shooting, Networking, Prediction, Physics, Lag compensation, Vehicles
- Classification: Deferred -> Adapted.
- Risk: Critical before the fix because ordinary guns predicted input state and presentation but never created a client projectile, so firing feedback, collision reports, server-projectile hiding, and lag-compensated correlation were disconnected; medium after the fix pending high-latency gameplay matrices.
- Behavior/API delta: CS-0266 correctly removed duplicate client/server `RequestShootEvent` firing owners, but the retained request schema no longer carried predicted projectile IDs or the client's last authoritative tick. `SharedGunPredictionSystem.ShootRequested` therefore returned an empty compatibility list, client `GunSystem.Shoot` only played effects and discarded ammo, and server projectiles never received `PredictedProjectileServerComponent`.
- Decision and behavior: Keep `SharedGunSystem` as the sole request owner. The client now pre-simulates the shot before raising the predictive request, creates real predicted projectile entities when `rmc.gun_prediction` permits, and includes their IDs plus `LastRealTick`. The first local request pass is skipped because it was already simulated; replays still apply shared gun and recoil state, and the server performs the one authoritative shot. Server projectiles receive fully initialized correlation components before component lifecycle events run. Recoil spread uses a deterministic tick-and-network-gun seed on both sides, and both the spread angle and last-fire timestamp are reconciled after rejected predictions. Prediction-ignore weapons retain immediate effects and authoritative firing without a client projectile or correlation marker; the server independently enforces that opt-out even if a client forges IDs.
- Safety and reconciliation: Both pre-simulation and authoritative request paths reject cross-map coordinates and targets. Client hit reporting ignores state application and cross-map/ignored contacts, requires an actual contact between the named projectile fixture and a hard compatible target fixture, selects the first valid collision, and marks each predicted projectile hit once. An authoritative copy is hidden only while its exact live client-side predicted copy exists, and that copy is retired when the server marker is removed; rejected and excess client IDs are explicitly acknowledged for cleanup. Physics replay also refuses to restore coordinates for deleting projectiles or invalid/deleting parents. Server hit reports must match the projectile's non-null map, lag-compensated target map, and reported-coordinate map, then pass bounds, masks, and two-sided `PreventCollideEvent` checks for the same named-projectile/hard-target fixture pair before `ProjectileCollide` can run. Invalid reports do not consume the projectile's one accepted-hit report, while the first accepted report ends processing for that projectile. The server prediction system's request listener only records lag-compensation timing and does not fire the gun.
- Files changed: `Content.Client/Weapons/Ranged/Systems/GunSystem.cs`, `Content.Client/_RMC14/Weapons/Ranged/Prediction/GunPredictionSystem.cs`, `Content.Server/Weapons/Ranged/Systems/GunSystem.cs`, `Content.Server/_RMC14/Weapons/Ranged/Prediction/GunPredictionSystem.cs`, `Content.Shared/Weapons/Ranged/Components/GunComponent.cs`, `Content.Shared/Weapons/Ranged/Events/RequestShootEvent.cs`, `Content.Shared/Weapons/Ranged/Systems/SharedGunSystem.cs`, `Content.Shared/_RMC14/Weapons/Ranged/Prediction/PredictedProjectileServerComponent.cs`, `Content.Shared/_RMC14/Weapons/Ranged/Prediction/SharedGunPredictionSystem.cs`, `Content.IntegrationTests/_RMC14/Weapons/Ranged/RMCGunPredictionTest.cs`, and `docs/upstream-sync/core-system-audit.md`.
- Validation evidence: The eight-case focused suite covers client projectile creation, predicted physics marking, deterministic client/server velocity, one-round authoritative consumption, exact projectile-ID correlation, a valid server network identity, authoritative cleanup, forged IDs against `GunIgnorePrediction`, explicit cleanup after a rejected shot, rollback-only recoil reconciliation followed by a matching accepted trajectory, cross-map forged-hit rejection, soft-only and mixed soft/hard-fixture rejection, IFF/`PreventCollideEvent` rejection, positive hostile hard-fixture acceptance, and exact predicted-copy matching. The focused suite passed 8/8 and the adjacent ranged regression selection passed 11/11. DebugOpt Shared, Client, and Server builds each succeeded with 0 warnings and 0 errors; exact-path `git diff --check` also passed.
- Remaining debt: Exercise semi/burst/full-auto fire under latency and packet loss; cartridge, magazine, battery, solution, shotgun, hitscan, thrown-ammo, remote, attachable, vehicle, and emplacement providers; moving/rotating grids; simultaneous shooters; CVar changes in flight; PVS exit/re-entry; mismatched/duplicate/oversized ID lists; lag-compensated hit boundary tolerances and multi-fixture contacts; reconnect, deletion, round restart, and long-running projectile cleanup. Xeno/custom prediction remains adjacent and unchanged.
