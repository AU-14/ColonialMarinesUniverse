...
public sealed class CMUVehicleSupportFootprintTest
{
    ...
    [Test]
    public void ReusesCallerBufferWithoutAllocating()
    {
        var samples = new List<Vector2>(16);
        var bounds = new Box2(-1f, -1f, 1f, 1f);

        for (var i = 0; i < 100; i++)
        {
            CMUVehicleSupportFootprint.GenerateLocalSamples(bounds, 1f, 0f, samples);
        }
