...
public sealed partial class YautjaCharacterProfile
{
    ...
    public static List<Marking> GetQuillMarkings(HumanoidCharacterAppearance appearance)
    {
        return appearance.Markings.TryGetValue("Head", out var head) &&
               head.TryGetValue(HumanoidVisualLayers.Hair, out var hair) ? hair : [];
    }

    public static Color GetQuillColor(HumanoidCharacterAppearance appearance)
    {
        return GetQuillMarkings(appearance).FirstOrDefault().MarkingColors?.FirstOrDefault() ?? appearance.SkinColor;
    }

    private static HumanoidCharacterAppearance BuildDefaultAppearance()
    {
        var skin = GetSkinColorColor(YautjaSkinColor.Green);
        return new HumanoidCharacterAppearance(GetEyeColorColor(YautjaEyeColor.Black), skin, new()
        {
            ["Head"] = new()
            {
    ...
    private static HumanoidCharacterAppearance ApplyQuillStyle(
        HumanoidCharacterAppearance appearance, YautjaQuillStyle style, Color? color = null)
    {
        var result = new HumanoidCharacterAppearance(appearance);
        if (!result.Markings.TryGetValue("Head", out var head))
            result.Markings["Head"] = head = new();
        var hair = GetQuillMarkings(result).Where(marking => !IsQuillMarking(marking.MarkingId)).ToList();
        hair.Add(new Marking(GetQuillMarkingId(style), [color ?? GetQuillColor(appearance)]));
        head[HumanoidVisualLayers.Hair] = hair;
        return result;
    }

    private static YautjaQuillStyle GetQuillStyle(HumanoidCharacterAppearance appearance)
    {
        foreach (var marking in GetQuillMarkings(appearance))
        {
