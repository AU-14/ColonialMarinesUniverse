...
public sealed partial class YautjaPowerSystem : EntitySystem
{
    ...
    private void OnEquipped(Entity<YautjaBracerComponent> ent, ref GotEquippedEvent args)
    {
        ...
        if (!ShouldSkipCmss13EquipAutoLock(ent.Comp, args.EquipTarget))
            SetLocked(ent, true);
        UpdateAlert(ent);
        AddBracerTacticalMapMarker(ent, args.EquipTarget);
    ...
    private void OnUnequipped(Entity<YautjaBracerComponent> ent, ref GotUnequippedEvent args)
    {
        ...
        ClearAlert(ent);
        RemoveBracerTacticalMapMarker(args.EquipTarget);
    ...
    private void AddBracerTacticalMapMarker(Entity<YautjaBracerComponent> bracer, EntityUid wearer)
    {
        ...
        marker.HadYautjaTracking = HasComp<Content.Shared.CMU14.TacticalMap.YautjaMapTrackedComponent>(wearer);
        marker.WasYautjaUser = TryComp<TacticalMapUserComponent>(wearer, out var oldUser) && oldUser.Yautja;
        marker.PreviousIcon = oldIcon?.Icon;
        marker.PreviousBackground = oldIcon?.Background;

        var yautja = HasComp<YautjaComponent>(wearer);
        // Current hunter prototypes already track dead hunters independently
        // of equipment. A temporary bracer marker must preserve that policy.
        var trackDead = TryComp<TacticalMapTrackedComponent>(wearer, out var oldTracking) && oldTracking.TrackDead;
        _tacticalMap.EnsureTracked(wearer, trackDead);
    ...
    private void RemoveBracerTacticalMapMarker(EntityUid wearer)
    {
        ...
        _tacticalMap.SetYautjaTracked(wearer, marker.HadYautjaTracking);
        _tacticalMap.SetYautjaUser(wearer, marker.WasYautjaUser);
