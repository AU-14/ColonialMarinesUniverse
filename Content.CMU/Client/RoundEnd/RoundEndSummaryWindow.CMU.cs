...
public sealed partial class RoundEndSummaryWindow
{
        ...
        RoundEndSummaryStats summaryStats)
    {
        ...
        var summaryContainer = new BoxContainer
        {
            Orientation = LayoutOrientation.Vertical,
            SeparationOverride = 14,
        };

        summaryContainer.AddChild(MakeReportHeader(gamemode, roundId, roundDuration, playersInfo));

        if (!string.IsNullOrWhiteSpace(roundEnd))
            summaryContainer.AddChild(MakeRoundEndTextPanel(roundEnd));

        summaryContainer.AddChild(MakeMetricSection(roundId, roundDuration, playersInfo));

        if (summaryStats.InjuryStats.Length == 0 && summaryStats.OddityStats.Length == 0)
        {
            summaryContainer.AddChild(MakeEmptyStatsPanel());
        }
        else
        {
            if (summaryStats.InjuryStats.Length > 0)
            {
                summaryContainer.AddChild(MakeStatSection(
        ...
        scrollContainer.AddChild(summaryContainer);
        roundEndSummaryTab.AddChild(scrollContainer);
        ...
        RoundEndPlayerInfo[] playersInfo)
    {
        ...
        panel.Name = "RoundEndAfterActionReport";
    ...
    private Control MakeStatSection(string title, string subtitle, RoundEndSummaryStat[] stats)
    {
        var section = new BoxContainer
        {
            Orientation = LayoutOrientation.Vertical,
    ...
    private Control MakeStatCard(RoundEndSummaryStat stat)
    {
        ...
        var panel = MakePanel(CardQuiet, accent.WithAlpha(AccentBorderAlpha));
        panel.Name = "RoundEndSummaryStatCard";
    ...
    private Control MakePlayerCard(RoundEndPlayerInfo playerInfo)
    {
        ...
        var panel = MakePanel(CardQuiet, accent.WithAlpha(AccentBorderAlpha));
        panel.Name = ManifestPlayerCardName;
    ...
    private Control MakePlayerSprite(RoundEndPlayerInfo playerInfo)
    {
    ...
    private static string GetPlayerRole(RoundEndPlayerInfo playerInfo)
    {
