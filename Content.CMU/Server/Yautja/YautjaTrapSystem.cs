...
public sealed partial class YautjaTrapSystem : EntitySystem
{
    ...
    {
        // Upstream animals now use slicing refinement instead of Butcherable.
        var butcherable = HasComp<ButcherableComponent>(uid) ||
                          TryComp<ToolRefinableComponent>(uid, out var refinable) && refinable.QualityNeeded == "Slicing";
        return butcherable &&
    ...
    {
        if (trap.Comp.ForceHumanPainEmote && HasComp<HumanoidProfileComponent>(tripper))
    ...
    {
        if (TryComp<YautjaTrapComponent>(trap, out var component))
            UpdateTrapVisibility((trap, component));
