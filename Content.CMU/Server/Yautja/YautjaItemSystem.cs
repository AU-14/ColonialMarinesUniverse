...
public sealed partial class YautjaItemSystem : EntitySystem
{
    ...
    private void OnFalconListen(Entity<YautjaFalconDroneComponent> drone, ref ListenEvent args)
    {
        if (!_containers.TryGetContainingContainer((drone.Owner, null, null), out var container) ||
            ...
            !HasComp<HumanoidProfileComponent>(args.Source) ||
        ...
        bool allowHeldBracer)
    {
        ...
        if (!HasComp<HumanoidProfileComponent>(user) || !CanUseYautjaTech(user))
        {
    ...
    {
        if (ent.Comp.Controller != args.EquipTarget ||
            !TryComp(args.EquipTarget, out YautjaFalconControllerComponent? controlling) ||
            controlling.SourceBracer != ent.Owner)
        {
            return;
        }

        CleanupFalconController(args.EquipTarget, controlling, deleteDrone: true, restoreEye: true);
    }
    ...
    {
        return HasComp<HumanoidProfileComponent>(user);
    }
