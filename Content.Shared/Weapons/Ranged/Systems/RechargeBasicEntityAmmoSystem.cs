...
public sealed partial class RechargeBasicEntityAmmoSystem : EntitySystem
{
    ...
    {
        ent.Comp.NextCharge = ent.Comp.StartWithCooldown
            ? _timing.CurTime + TimeSpan.FromSeconds(ent.Comp.RechargeCooldown)
            : _timing.CurTime;
        Dirty(ent);
    }
    ...
    public void Reset(Entity<RechargeBasicEntityAmmoComponent?> ent)
    {
        ...
        if (ent.Comp.NextCharge == null ||
            ent.Comp.ResetOverdueCooldown && ent.Comp.NextCharge < _timing.CurTime)
        {
