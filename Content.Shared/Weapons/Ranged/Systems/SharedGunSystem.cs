...
using Content.Shared.CMU14.Weapons.Ranged; // CMU14
...
public abstract partial class SharedGunSystem : EntitySystem
{
    ...
    private void OnGunMelee(Entity<GunComponent> ent, ref MeleeHitEvent args)
    {
        ...
        if (ent.Comp.MeleeCooldownOnShoot && melee.NextAttack > ent.Comp.NextFire)
        {
    ...
    /// <summary>
    /// Tries to get a gun from an RMC vehicle/controller, the entity's hands, or the entity itself.
    /// </summary>
    public bool TryGetGun(EntityUid entity, out Entity<GunComponent> gun)
    {
        ...
        if (_attachableHolder.TryGetInhandSupercedingGun(entity, out var attachable, out var attachableGun))
        {
        ...
        // RMC14 controlled emplacements.
        if (_rmcSharedWeaponController.TryGetControlledWeapon(entity, out var weapon, out var controlledGun) &&
            weapon is { } controlledWeapon)
        {
            gun = (controlledWeapon, controlledGun);
            return true;
        }
    ...
    public List<EntityUid>? AttemptShoot(Entity<GunComponent> ent, EntityUid user, EntityCoordinates coordinates)
    {
        ...
        var projectiles = AttemptShoot(user, ent);
        ent.Comp.ShotCounter = 0;
        ...
        ICommonSession? userSession = null)
    {
        ...
        if (gun.Comp.NextFire < curTime - fireRate ||
            gun.Comp.ShotCounter == 0 && gun.Comp.NextFire < curTime)
        {
        ...
        if (attemptEv.Cancelled)
        {
            ...
            gun.Comp.NextFire = attemptEv.ResetCooldown
                ...
                : TimeSpan.FromSeconds(Math.Max(
                    lastFire.TotalSeconds + SafetyNextFire,
                    gun.Comp.NextFire.TotalSeconds));
            return null;
        }
        ...
        if (ev.Ammo.Count <= 0)
        {
            ...
            if (shots > 0)
            {
                ...
                gun.Comp.NextFire = TimeSpan.FromSeconds(Math.Max(
                    lastFire.TotalSeconds + SafetyNextFire,
                    gun.Comp.NextFire.TotalSeconds));
        ...
        foreach (var (ammoEntity, _) in ev.Ammo)
        {
            ...
            if (IsClientSide(ammoEntity.Value) &&
                (HasComp<GunIgnorePredictionComponent>(gun) ||
                 projectiles == null ||
                 !projectiles.Contains(ammoEntity.Value)))
            {
        ...
        ICommonSession? userSession = null)
    {
        ...
        var ammoList = new List<(EntityUid? Entity, IShootable Shootable)>(1);
        ammoList.Add((ammo, shootable));
        return Shoot(
            gun,
            ammoList,
            fromCoordinates,
    ...
    protected Angle GetRecoilAngle(Entity<GunComponent> gun, Angle direction)
    {
        var timeSinceLastFire = (Timing.CurTime - gun.Comp.LastFire).TotalSeconds;
        ...
        seed = seed << 32;
        seed |= (uint) GetNetEntity(gun.Owner).Id;
    ...
    protected void PlayGunshotSound(SoundSpecifier? sound, EntityUid gun, EntityUid? user)
    {
        if (!_zLevels.PlayPredictedDirectlyAcrossZ(sound, gun, user))
            Audio.PlayPredicted(sound, gun, user);
    }
