...
public abstract partial class SharedChatSystem
{
    ...
    )
    {
        ...
        if (didEmote && emote.ChatMessages.Count != 0)
        {
            ...
            GetEmotePresentation(emote, out var speechBubbleMessage, out var speechStyleClass);
            SendEntityEmote(
                source,
                action,
                range,
    ...
    public bool TryPlayEmoteSound(EntityUid uid, EmoteSoundsPrototype? proto, string emoteId, AudioParams? audioParams = null)
    {
        ...
        {
            var filter = GetEmoteSoundFilter(uid);
            if (filter.Count == 0)
                return false;
    ...
    protected bool TryEmoteChatInput(EntityUid source, string textInput)
    {
        ...
        if (!CanInvokeChatEmote(source, emote))
            return true;
    ...
    /// <summary>
    /// Allows platform-specific chat systems to customize an emote's runechat presentation.
    /// </summary>
    protected virtual void GetEmotePresentation(
        EmotePrototype emote,
    ...
    /// <summary>
    /// Allows platform-specific chat systems to suppress a typed emote event while still displaying its chat text.
    /// </summary>
    protected virtual bool CanInvokeChatEmote(EntityUid source, EmotePrototype emote)
    {
        return true;
    }

    /// <summary>
    /// Selects recipients for a server-side emote sound.
    /// </summary>
    protected virtual Filter GetEmoteSoundFilter(EntityUid source)
    {
