...
public sealed partial class XenoScreechSystem : EntitySystem
{
    ...
    private void OnXenoScreechAction(Entity<XenoScreechComponent> xeno, ref XenoScreechActionEvent args)
    {
        ...
        if (attempt.Cancelled || !_xenoPlasma.TryRemovePlasmaPopup(xeno.Owner, xeno.Comp.PlasmaCost))
            return;
        ...
        foreach (var receiver in _mobs)
        {
            if (!_xeno.CanAbilityAttackTarget(xeno, receiver) ||
                !ApplyScreechEffects(xeno, receiver, xeno.Comp.SlowTime, xeno.Comp.BlindTime))
            {
                continue;
            }
    ...
    {
        if (_mobState.IsDead(receiver) || !_examine.InRangeUnOccluded(xeno, receiver))
            return false;
    ...
    private void RemoveScatterFromTrackedGuns(ScreechBlindComponent blind)
    {
