...
public sealed partial class XenoChargerCollisionSystem : EntitySystem
{
        ...
        EntityUid target)
    {
        ...
        if (HasComp<VehicleComponent>(target))
        {
            ...
            if (isCharged && TryComp(target, out VehicleInteriorComponent? interior))
            {
                foreach (var occupant in interior.Passengers.Concat(interior.Xenos))
                {
            ...
            wallPhysics.Hard && wallPhysics.BodyType == BodyType.Static)
        {
            ...
            var damage = new DamageSpecifier();
            damage.DamageDict[_blunt] = (stage + 1) * xeno.ChargedDamageBase;
            var changed = _damageable.TryChangeDamage(target, damage, out var damageChanged);

            if (!changed || damageChanged.GetTotal() <= FixedPoint2.Zero)
            {
                _movement.ResetToIdle(charger);
                return;
            }
