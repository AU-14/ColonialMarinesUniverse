...
public sealed partial class BloodstreamSystem : EntitySystem
{
    ...
    private void OnMapInit(Entity<BloodstreamComponent> entity, ref MapInitEvent args)
    {
        ...
        // Fill every reference reagent independently so foreign chemicals already in the unified solution do not
        // displace the entity's initial blood.
        // The DNA string might not be initialized yet, but the reagent data gets updated in the GenerateDnaEvent subscription.
        foreach (var (referenceReagent, referenceQuantity) in entity.Comp.BloodReferenceSolution)
        {
            var present = bloodSolution.Comp.Solution.GetTotalPrototypeQuantity(referenceReagent.Prototype);
            if (referenceQuantity > present)
                bloodSolution.Comp.Solution.AddReagent(referenceReagent, referenceQuantity - present);
        }
    }
    ...
    private void OnDnaGenerated(Entity<BloodstreamComponent> entity, ref GenerateDnaEvent args)
    {
        if (_solutionContainer.ResolveSolution(entity.Owner, entity.Comp.BloodSolutionName, ref entity.Comp.BloodSolution, out var bloodSolution))
        {
            ...
            {
                if (!entity.Comp.BloodReferenceSolution.ContainsPrototype(reagent.Reagent.Prototype))
                    continue;
    ...
    private void OnDamageChanged(Entity<BloodstreamComponent> ent, ref DamageChangedEvent args)
    {
        ...
        if (totalFloat > 0
            && !_mobStateSystem.IsDead(ent)
            && SharedRandomExtensions.PredictedProb(_timing, prob, GetNetEntity(ent), GetNetEntity(args.Origin)))
        {
    ...
    {
        if (HasComp<RMCMedicalExamineComponent>(ent))
            return;
        ...
        // If the mob's blood level is below the damage threshold, the pale message is added.
    ...
    /// <summary>
    /// Returns whether the configured reference blood contains the requested reagent prototype.
    /// </summary>
    public bool HasReferenceReagent(
        ...
        ProtoId<ReagentPrototype> reagent)
    {
        ...
        foreach (var (reference, _) in entity.Comp.BloodReferenceSolution)
        {
            if (reference.Prototype == reagent)
                return true;
        }
    ...
    /// <summary>
    /// Gets the first configured reference-blood reagent for deterministic visual effects.
    /// </summary>
    public bool TryGetPrimaryReferenceReagent(
        Entity<BloodstreamComponent?> entity,
        out ProtoId<ReagentPrototype> reagent)
    {
        reagent = default;
        if (!Resolve(entity, ref entity.Comp, logMissing: false)
            || entity.Comp.BloodReferenceSolution.Contents.Count == 0)
        {
            return false;
        }

        reagent = entity.Comp.BloodReferenceSolution.Contents[0].Reagent.Prototype;
        return true;
    }

    /// <summary>
    /// Gets all configured reference-blood reagent prototypes.
    /// </summary>
    public ProtoId<ReagentPrototype>[] GetReferenceReagentPrototypes(Entity<BloodstreamComponent?> entity)
    {
        if (!Resolve(entity, ref entity.Comp, logMissing: false))
            return [];

        var references = new ProtoId<ReagentPrototype>[entity.Comp.BloodReferenceSolution.Contents.Count];
        for (var i = 0; i < references.Length; i++)
        {
            references[i] = entity.Comp.BloodReferenceSolution.Contents[i].Reagent.Prototype;
        }
    ...
    /// <summary>
    /// Gets the configured normal reference-blood volume.
    /// </summary>
    public FixedPoint2 GetReferenceVolume(Entity<BloodstreamComponent?> entity)
    {
        if (!Resolve(entity, ref entity.Comp, logMissing: false))
            return FixedPoint2.Zero;

        return entity.Comp.BloodReferenceSolution.Volume;
    }

    /// <summary>
    /// Gets an isolated copy of the configured reference-blood solution.
    /// </summary>
    public Solution? GetBloodReferenceSolution(Entity<BloodstreamComponent?> entity)
    {
        if (!Resolve(entity, ref entity.Comp, logMissing: false))
            return null;

        return entity.Comp.BloodReferenceSolution.Clone();
    }
    ...
    public bool TryRegulateBloodLevel(Entity<BloodstreamComponent?> ent, FixedPoint2 amount, float referenceFactor = 1f)
    {
        if (!Resolve(ent, ref ent.Comp, logMissing: false)
            || !_solutionContainer.ResolveSolution(ent.Owner, ent.Comp.BloodSolutionName, ref ent.Comp.BloodSolution, out var bloodSolution)
            || amount == 0
            || ent.Comp.BloodReferenceSolution.Volume <= FixedPoint2.Zero)
            return false;
    ...
    public bool TryBleedOut(Entity<BloodstreamComponent?> ent, FixedPoint2 amount)
    {
        ...
        var bloodSolution = ent.Comp.BloodSolution.Value.Comp.Solution;
        var referenceReagents = GetReferenceReagentPrototypes(ent);
        var leakedBlood = bloodSolution.SplitSolutionWithOnly(amount, referenceReagents);
        _solutionContainer.UpdateChemicals(ent.Comp.BloodSolution.Value);

        if (!_solutionContainer.ResolveSolution(
                ent.Owner,
                ent.Comp.BloodTemporarySolutionName,
                ref ent.Comp.TemporarySolution,
        ...
        {
            if (ent.Comp.SpillChemicals)
            {
                var leakedChemicals = bloodSolution.SplitSolutionWithout(tempSolution.Volume / 10, referenceReagents);
                tempSolution.AddSolution(leakedChemicals, ProtoMan);
                _solutionContainer.UpdateChemicals(ent.Comp.BloodSolution.Value);
            }
    ...
    public void ChangeBloodReagents(Entity<BloodstreamComponent?> ent, Solution reagents)
    {
        ...
        if (currentVolume == FixedPoint2.Zero || ent.Comp.BloodReferenceSolution.Volume <= FixedPoint2.Zero)
            return;
    ...
    public void CopyComponent(Entity<BloodstreamComponent?> source, EntityUid target)
    {
        ...
        cloneComp.SpillChemicals = source.Comp.SpillChemicals;
