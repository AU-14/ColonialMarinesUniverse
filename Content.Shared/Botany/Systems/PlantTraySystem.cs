    ...
    public void UpdateReagents(Entity<PlantTrayComponent?> ent)
    {
        ...
        foreach (var entry in contents)
        {
            ...
            var reagentContext = new ReagentEffectContext(
                reagentProto,
                solution,
                trayComp.SoilSolution.Value.Owner,
                null,
                entry,
                null,
                null,
            ...
            _entityEffects.ApplyEffects(
                trayUid,
                [.. reagentProto.PlantMetabolisms],
                entry.Quantity.Float(),
                reagentContext: reagentContext);
            _entityEffects.ApplyEffects(
                plantUid.Value,
                [.. reagentProto.PlantMetabolisms],
                entry.Quantity.Float(),
        ...
        var removed = _solutionContainer.RemoveEachReagent(
            trayComp.SoilSolution.Value,
            FixedPoint2.New(1));

        // The legacy hydroponics path invoked chemical-property effects with the amount actually
        // removed this tick, while the upstream plant effects intentionally scale with the full
        // quantity in the tray. Keep those two quantities distinct.
        foreach (var entry in removed.Contents)
        {
            var reagentProto = _reagent.Index(entry.Reagent.Prototype);
            if (reagentProto.Metabolisms is null)
                continue;
            ...
            foreach (var metabolism in reagentProto.Metabolisms.Metabolisms.Values)
            {
                foreach (var effect in metabolism.Effects)
                {
                    if (effect is RMCChemicalEffect)
                    {
