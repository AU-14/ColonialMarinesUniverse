...
public sealed partial class MetabolizerSystem : EntitySystem
{
    ...
    private void TryMetabolizeStage(Entity<MetabolizerComponent, OrganComponent?, SolutionManagerComponent?> ent, ProtoId<MetabolismStagePrototype> stage)
    {
        ...
        foreach (var (reagent, quantity) in list)
        {
            ...
            var rate = solutionData.MetabolizeAll
                ? quantity
                : entry.MetabolismRate * (FixedPoint2) MathF.Max(0f, rateEvent.Multiplier);
            ...
            void ApplyEffect(EntityEffect effect)
            {
                ...
                {
                    case ModifyLungGas:
                        _entityEffects.ApplyEffect(ent, effect, scale, reagentContext: reagentContext);
                        break;
                    case AdjustReagent:
                        _entityEffects.ApplyEffect(
                            solutionEntity.Value,
                            effect,
                            scale,
                        ...
                        break;
                    default:
                        _entityEffects.ApplyEffect(actualEntity, effect, scale, reagentContext: reagentContext);
                        break;
                }
            }
    ...
    private void TryMetabolize(Entity<MetabolizerComponent, OrganComponent?, SolutionManagerComponent?> ent)
    {
        ...
        if (!_stasis.CanOrganMetabolize((ent.Owner, ent.Comp2)))
            return;
        ...
        foreach (var condition in conditions)
        {
            switch (condition)
            {
                ...
                case ReagentCondition reagentCondition:
                    var result = _reagentCondition.Check(
