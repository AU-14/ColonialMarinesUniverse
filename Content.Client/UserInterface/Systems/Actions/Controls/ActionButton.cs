...
public sealed class ActionButton : Control, IEntityControl
{
    ...
    private Texture? _toggledBackground;
    ...
    public void UpdateIcons()
    {
        UpdateItemIcon();
        UpdateActionIcon();
        UpdateToggledBackground();
        UpdateBackground();
    }

    private void UpdateToggledBackground()
    {
        _toggledBackground = null;

        if (Action?.Comp.BackgroundOn is not { } background)
            return;

        _spriteSys ??= _entities.System<SpriteSystem>();
        _toggledBackground = _spriteSys.Frame0(background);
    }
    ...
    public void UpdateBackground()
    {
            ...
            _controller.IsDragging && GetPositionInParent() == Parent?.ChildCount - 1)
        {
            Button.Texture = Action is { } action &&
                             _toggledBackground != null &&
                             (action.Comp.Toggled || _controller.SelectingTargetFor == action.Owner)
                ? _toggledBackground
                : _slotBackground;
        }
        else
