...
public sealed partial class HumanoidAppearanceSystem : SharedHumanoidAppearanceSystem
{
    ...
    public override void LoadProfile(EntityUid uid, HumanoidCharacterProfile? profile, HumanoidAppearanceComponent? humanoid = null)
    {
        ...
        foreach (var (_, organMarkings) in profile.Appearance.Markings)
        {
            foreach (var (layer, layerMarkings) in organMarkings)
            {
                // Hair remains handled below through the legacy compatibility properties.
    ...
    {
        if (undergarmentTop && humanoid.UndergarmentTop is { } undergarmentTopId)
        {
            var marking = new Marking(undergarmentTopId, new List<Color> { new Color() });
            if (_markingManager.TryGetMarking(marking, out var prototype))
            {
        ...
        if (undergarmentBottom && humanoid.UndergarmentBottom is { } undergarmentBottomId)
        {
            var marking = new Marking(undergarmentBottomId, new List<Color> { new Color() });
            if (_markingManager.TryGetMarking(marking, out var prototype))
            {
