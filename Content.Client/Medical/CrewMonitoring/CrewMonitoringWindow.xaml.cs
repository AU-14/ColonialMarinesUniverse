...
public sealed partial class CrewMonitoringWindow : FancyWindow
{
    ...
    private readonly Dictionary<NetEntity, Color> _sensorBlipColors = new();
    ...
    public CrewMonitoringWindow()
    {
        ...
        {
            if (_lastSensors != null)
                ShowSensors(_lastSensors, _lastMonitor, _lastMonitorCoords);
        };
    }
    ...
    public void ShowSensors(List<SuitSensorStatus> sensors, EntityUid monitor, EntityCoordinates? monitorCoords)
    {
        ...
        var uniqueSensors = uniqueSensorsMap.Values.Where(sensor => CrewMonitoringFilter.Matches(sensor, SearchLineEdit.Text)).ToList();
    ...
    private void PopulateDepartmentList(IEnumerable<SuitSensorStatus> departmentSensors)
    {
        ...
        foreach (var sensor in departmentSensors)
        {
            ...
            if (sensor.OxygenDamage is { } oxygen && sensor.ToxinDamage is { } toxin &&
                sensor.BurnDamage is { } burn && sensor.BruteDamage is { } brute)
            {
                sensorButton.ToolTip = Loc.GetString("cmu-yautja-crew-monitor-tooltip",
                    ("area", sensor.Area ?? Loc.GetString("cmu-yautja-crew-monitor-area-unknown")),
                    ("oxygen", oxygen),
                    ("toxin", toxin),
                    ("burn", burn),
                    ("brute", brute));
            ...
            {
                var blipColor = GetSensorBlipColor(sensor);
                _sensorBlipColors[sensor.SuitSensorUid] = blipColor;
            ...
            if (NavMap.TrackedEntities.TryGetValue(castSensor.SuitSensorUid, out var data))
            {
                data = new NavMapBlip
                    ...
                    (currTrackedEntity == null || castSensor.SuitSensorUid == currTrackedEntity)
                        ? _sensorBlipColors.GetValueOrDefault(castSensor.SuitSensorUid, Color.LimeGreen)
                        : _sensorBlipColors.GetValueOrDefault(castSensor.SuitSensorUid, Color.LimeGreen) * Color.DimGray,
    ...
    private void ClearOutDatedData()
    {
        ...
        _sensorBlipColors.Clear();
    }

    private static Color GetSensorBlipColor(SuitSensorStatus sensor)
    {
        return sensor.LocationKind switch
        {
            YautjaCrewMonitoringLocationKind.MainShip => Color.CornflowerBlue,
            YautjaCrewMonitoringLocationKind.HuntingGround => Color.Red,
            _ => Color.LimeGreen,
        };
    }
}
