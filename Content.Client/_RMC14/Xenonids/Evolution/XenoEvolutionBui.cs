...
public sealed partial class XenoEvolutionBui : BoundUserInterface
{
    ...
    public void Refresh()
    {
        ...
        _window.PointsLabel.Text = Loc.GetString("rmc-xeno-ui-evolution-points", // CMU14
        ...
        // CMU14: Determine which "lacking" label to show
        bool showOviLabel = false;
        bool showOvermindLabel = false;

        if (lackingOvipositor) // CMU14
        {
            if (IsPathogenHive())
        ...
        // CMU14: OvermindNeededLabel - add this RichTextLabel to XenoEvolutionWindow.xaml alongside OvipositorNeededLabel
    ...
    private bool IsPathogenHive() // CMU14 method
    {
        if (!EntMan.TryGetComponent(Owner, out HiveMemberComponent? member) ||
            member.Hive is not { } hive ||
            !EntMan.TryGetComponent(hive, out HiveComponent? hiveComp))
    ...
    private bool PathogenHiveHasLivingOvermind() // CMU14 method
    {
        if (!EntMan.TryGetComponent(Owner, out HiveMemberComponent? member) ||
            member.Hive is not { } hive)
        {
