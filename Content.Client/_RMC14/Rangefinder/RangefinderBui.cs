...
public sealed class RangefinderBui : BoundUserInterface
{
    ...
    public void Refresh()
    {
        ...
        if (rangefinder.LastCoords is { } mapCoords)
        {
            ...
            var canMortar = _topDownOrdnance.TryResolveImpactColumn(
                mapCoords,
                CMUTopDownOrdnanceKind.Mortar,
                out var mortar);
            var canOrbital = _topDownOrdnance.TryResolveImpactColumn(
                mapCoords,
            ...
            _window.BottomContainer.AddChild(AddRow(GetOrdnanceLabel(
                Loc.GetString("rmc-rangefinder-orbital-bombardment"),
                orbital), canOrbital));
        }
    }
    ...
    {
        return result is { Redirected: true } ? $"{label} (top-down)" : label;
    }
