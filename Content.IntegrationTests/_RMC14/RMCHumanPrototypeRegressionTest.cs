...
public sealed class RMCHumanPrototypeRegressionTest
{
    ...
    public async Task CmuDetachedOrgansLeavePrivateVisibilityLayer()
    {
        ...
        EntityUid parentPart = default;
        string organSlot = string.Empty;
        var internalLayer = (ushort) VisibilityFlags.CMUMedicalInternals;
        ...
        await server.WaitAssertion(() =>
        {
            ...
            parentPart = body.GetParentPartOrNull(organ) ?? default;
            var organComponent = entMan.GetComponent<OrganComponent>(organ);
            organSlot = SharedBodySystem.GetCanonicalSlotId(organComponent.Category) ?? string.Empty;
            Assert.Multiple(() =>
            {
                Assert.That(parentPart, Is.Not.EqualTo(default(EntityUid)));
                Assert.That(organSlot, Is.Not.Empty);
            });
        ...
        await server.WaitAssertion(() =>
        {
            ...
            Assert.That(body.InsertOrgan(parentPart, organ, organSlot), Is.True);
        });
