    ...
    public sealed class StorageTest
    {
        ...
        public async Task StorageSizeArbitrageTest()
        {
            ...
            await server.WaitAssertion(() =>
            {
                ...
                {
                    if (!proto.TryComp(out StorageComponent? storage, compFact) ||
                        storage.Whitelist != null ||
                        storage.MaxItemSize == null ||
                        !proto.TryComp(out ItemComponent? item, compFact))
                        continue;
        ...
        public async Task TestStorageFillPrototypes()
        {
            ...
            await server.WaitAssertion(() =>
            {
                Assert.Multiple(() =>
                {
                    foreach (var proto in protoManager.EnumeratePrototypes<EntityPrototype>())
                    {
                        if (!proto.TryComp(out StorageFillComponent? storage, compFact))
                            continue;
            ...
            await Assert.MultipleAsync(async () =>
            {
                foreach (var (proto, fill) in pair.GetPrototypesWithComponent<StorageFillComponent>())
                {
                    ...
                    await server.WaitAssertion(() =>
                    {
                        ...
                        proto.TryComp(out ignoreContentsSize, compFact); // RMC14
            ...
            IComponentFactory componentFactory)
        {
            ...
            if (proto.TryComp(out ItemComponent? item, componentFactory))
            ...
            IComponentFactory componentFactory)
        {
            ...
            foreach (var entry in fill.Contents)
            {
                var size = GetEntrySize(entry, getCount, protoMan, itemSystem, componentFactory);
