...
## 18. Проверка обратной связи PR #1648 (2026-08-08)

Подтверждённые исправления после сверки с CMSS13/RMC/CMU:

- Medicomp: три timed-этапа используют одну стабильную DoAfter-сессию (`StartAndEnd`), поэтому прогрессбар больше не пересоздаётся и не мигает. Healing gun, stabilizer gel и wound clamp проходят полный цикл; healing capsules остаются дискретными предметами, а военный контейнер трав исключён из whitelist Medicomp.
- Медицинские названия и ресурсы приведены к Yautja-локалям и `medical.rsi`; thwei-лечение и закрытие внешнего кровотечения покрыты parity-тестами.
- Военная маска получила тот же health HUD, что и обычная маска. Для оружия на спине используются только directional back-спрайты; одиночные world-иконки больше не перекрывают тело Yautja.
- Deploy actions bracer показываются только для реально установленного attachment. Ролевые военные bracers не заменяются общим bracer при применении профиля.
- Посадка Hunter Ship dropship на A/B beacon и Hangar сохраняет destination в локальных координатах Hunter Ship grid во время FTL и вычисляет актуальную map-позицию только при прибытии. A/B beacon задают явный offset от центра marker-тайла к origin сетки шаттла; shuttle сохраняет отдельный static/fixed-rotation grid карты. Обычные shuttle destinations продолжают использовать proximity fallback. Регрессионные тесты проверяют A/B/Hangar после движения и поворота Hunter Ship, а также возврат round-start шаттла в его исходную позу.
- Yautja communicator использует свободный английский канал `:g`; общий канал Abomination перенесён на `:9`, чтобы глобальный chat lookup не перехватывал `:g`. Коммуникатор и comms tower проверены runtime-тестом.
- Self-destruct action разрешён в critical состоянии, а damage/leap/audio регрессии проверяются отдельными Yautja-тестами.

Финальная проверка: `Content.IntegrationTests` — 22 целевых теста пройдены, Hunter Ship dropship landing — 1/1 пройден; сборка server/client — 0 ошибок. Сервер и клиент запущены из `bin/Content.Server` и `bin/Content.Client`.

## 19. Additional weapon and communicator verification

- Back-slot Yautja melee weapons now use directional `pred_gear_worn.rsi` states ported from CMSS13's on-mob `pred_gear.dmi`; CMU-only and previously duplicated CMSS13 variants use unique generated four-direction layers. Firearms retain directional `pred_guns_back.rsi` overlays.
- The military communicator is verified after map initialization to show `:m for YSPEC` in examine. The general Yautja channel remains the dedicated `:g` key.
- Regression coverage is provided by `YautjaWornWeaponVisualTest`, the full held/back directional-frame checks, and `MilitaryCommunicatorExamineListsYspecOnMilitaryPrefix`.
