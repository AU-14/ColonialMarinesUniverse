...
public sealed partial class AntagSelectionSystem
{
        ...
        GetAntagJobs(params HashSet<ProtoId<AntagSpecifierPrototype>> except)
    {
        ...
        while (query.MoveNext(out var uid, out var comp, out _))
        {
            ...
            foreach (var antag in comp.Antags)
            {
                ...
                var jobBlacklist = GetJobBlacklist(proto);

                // Check this here so we don't make a dictionary entry for a bunch of players, with empty blacklists and whitelists.
                if (jobBlacklist == null && proto.JobWhitelist == null)
                    continue;

                foreach (var player in set)
                {
                    result.TryGetValue(player, out var jobs);
                    var whitelist = jobs.Whitelist;
                    var blacklist = jobs.Blacklist;

                    if (proto.JobWhitelist != null)
                    {
                        whitelist ??= new HashSet<ProtoId<JobPrototype>>();
                        whitelist.UnionWith(proto.JobWhitelist);
                    }

                    if (jobBlacklist != null)
                    {
                        blacklist ??= new HashSet<ProtoId<JobPrototype>>();
                        blacklist.UnionWith(jobBlacklist);
                    }

                    result[player] = (whitelist, blacklist);
                }
            }
        }
        ...
        GetAntagJobs(ICommonSession player, params HashSet<ProtoId<AntagSpecifierPrototype>> except)
    {
        ...
        while (query.MoveNext(out var uid, out var comp, out _))
        {
            ...
            foreach (var antag in comp.Antags)
            {
                ...
                if (GetJobBlacklist(proto) is { } jobBlacklist)
                {
                    blacklist ??= new HashSet<ProtoId<JobPrototype>>();
                    blacklist.UnionWith(jobBlacklist);
                }
    ...
    private HashSet<ProtoId<JobPrototype>>? GetJobBlacklist(AntagSpecifierPrototype prototype)
    {
        HashSet<ProtoId<JobPrototype>>? blacklist = prototype.JobBlacklist == null
            ? null
            : new HashSet<ProtoId<JobPrototype>>(prototype.JobBlacklist);

        if (prototype.JobBlacklistGroup == null)
            return blacklist;

        foreach (var groupId in prototype.JobBlacklistGroup)
        {
            if (!ProtoMan.TryIndex(groupId, out AntagJobBlacklistPrototype? group))
                continue;
    ...
    public IEnumerable<ProtoId<AntagPrototype>> GetValidAntagPreferences(ICommonSession session, List<ProtoId<AntagPrototype>>? filter = null)
    {
        ...
        var presetId = GameTicker.CurrentPreset?.ID ?? GameTicker.Preset?.ID;
        foreach (var antag in prefs.SelectedCharacter.GetAntagPreferencesForGamemode(presetId))
        {
