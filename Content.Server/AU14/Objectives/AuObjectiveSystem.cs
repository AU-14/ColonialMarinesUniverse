...
public sealed partial class AuObjectiveSystem : AuSharedObjectiveSystem
{
    ...
    // TODO: this does nothing but DEBUG logging
    ...
    private void OnPostGameMapLoad(PostGameMapLoad ev)
    {
        ...
        foreach (var grid in ev.Grids)
        {
            if (TryComp<MapGridComponent>(grid, out var gridComp))
            {
                ...
                if (area > bestArea) { bestArea = area; bestPlanetGrid = grid; }
            }
        }

        if (bestPlanetGrid == null) { _logs.Warning($"[OBJ MASTER] OnPostGameMapLoad: planet map has no valid grids!"); return; }
        ...
        var masterScan = EntityQueryEnumerator<ObjectiveMasterComponent>();
        while (masterScan.MoveNext(out var mUid, out var mComp))
        {
            var scopeMatch = mComp.MapScope == MasterMapScope.Planet
                || (mComp.MapScope == MasterMapScope.Unset && _rmcPlanetSystem.IsOnPlanet(Transform(mUid)));
            if (scopeMatch && mComp.GamePreset.Equals(presetId, StringComparison.OrdinalIgnoreCase))
            {
        ...
        if (TryComp<ObjectiveMasterComponent>(_objectiveMasterUid, out var master))
            _fetchObjectiveSystem.SpawnMissingFetchObjectives(presetId, new EntityCoordinates(bestPlanetGrid.Value, Vector2.Zero), master, _allObjectives, _proto, _logs);
        Timer.Spawn(0, Main);
    }
        ...
        int scientistMinor, int scientistMajor) ObjectivesAmount()
    {
        if (GetOrReselectObjMaster() is not { } master)
        {
            _logs.Warning("[OBJ AMT] ObjectivesAmount called with null ObjectiveMaster!");
    ...
    public void CompleteObjectiveForFaction(EntityUid uid, AuObjectiveComponent objective, string completingFaction)
    {
        ...
        {
            _logs.Error("[OBJ COMPLETE] CompleteObjectiveForFaction called with null ObjectiveMaster!");
            return;
        }
    ...
    private List<(EntityUid Uid, AuObjectiveComponent Comp)> GetInactiveObjectives(string? presetId = null)
    {
        ...
        foreach (var (uid, comp) in _allObjectives)
        {
            ...
            // if (presetId != null && !comp.ApplicableModes.Contains(presetId, StringComparer.OrdinalIgnoreCase))
            //     continue;
            bool modeMatch = true;
            if (presetId != null)
            {
    ...
    private void ApplyWinPoints(string faction, int points)
    {
        if (GetOrReselectObjMaster() is not { } master)
        {
            _logs.Error("[OBJ WIN] ApplyWinPoints called with null ObjectiveMaster!");
            return;
        }
    ...
    private bool TrySelectMasterComponent(string? presetId)
    {
        ...
        {
            if (selected == null) { selected = (uid, comp); continue; }
            if (IsBetterMaster(uid, comp, selected.Value.Uid, selected.Value.Comp, presetId))
                selected = (uid, comp);
        }

        if (selected == null)
        {
            _logs.Error("[OBJ MASTER]     Cannot find an ObjectiveMaster to select!");
            return false;
        }

        if (!selected.Value.Comp.GamePreset.Equals(presetId, StringComparison.OrdinalIgnoreCase))
            _logs.Warning($"[OBJ MASTER] No ObjectiveMaster found for preset '{presetId}'. Falling back to '{selected.Value.Comp.GamePreset}' master (uid {selected.Value.Uid}).");

        _objectiveMasterUid = selected.Value.Uid;
        return true;
    }

    private bool IsBetterMaster(
    EntityUid candidateUid, ObjectiveMasterComponent candidate,
    EntityUid selectedUid, ObjectiveMasterComponent current,
    string? presetId)
    {
        _logs.Debug($"[OBJ MASTER]     Comparison IsBetterMaster: candidate ({candidateUid}) vs. selected ({selectedUid})");
        // TODO: rewrite this a bit tigther - testing functionality
        var bothLegacy = candidate.MapScope == MasterMapScope.Unset && current.MapScope == MasterMapScope.Unset;
        if (!bothLegacy)
        {
            // candidate has explicit Planet scope -> prefer candidate
            if (candidate.MapScope == MasterMapScope.Planet && current.MapScope != MasterMapScope.Planet)
                return true;
            // current has explicit Planet scope -> keep current
            if (candidate.MapScope != MasterMapScope.Planet && current.MapScope == MasterMapScope.Planet)
                return false;
            // same scope -> game preset match
            if (candidate.MapScope == current.MapScope)
                return candidate.GamePreset.Equals(presetId, StringComparison.OrdinalIgnoreCase)
                    && !current.GamePreset.Equals(presetId, StringComparison.OrdinalIgnoreCase);
            return false; // mixed explicit -> no preference
        }

        // legacy fallback -> uses IsOnPlanet
        var candOnPlanet = _rmcPlanetSystem.IsOnPlanet(Transform(candidateUid));
        var selOnPlanet = _rmcPlanetSystem.IsOnPlanet(Transform(selectedUid));
        if (candOnPlanet != selOnPlanet)
            return candOnPlanet;
        // same planet? -> game preset match
        return candidate.GamePreset.Equals(presetId, StringComparison.OrdinalIgnoreCase)
            && !current.GamePreset.Equals(presetId, StringComparison.OrdinalIgnoreCase);
    }
    ...
    public void Main()
    {
        ...
        {
            _logs.Warning("[OBJ MAIN]   No objectives passed filtering for DistressSignal:");
            foreach (var (uid, comp) in _allObjectives.Take(30))
        ...
        // Old method of selecting, left for reference - it did the try catch blocks below nicely
        ...
        foreach (var faction in factions)
        {
            try
            {
                ...
                ActivateFactionObjectives(faction, 1,
                    SelectObjectives(faction, modeObjectives, 1, GetRandomObjectiveCount(maxMinor, minMinor)));
                ActivateFactionObjectives(faction, 2,
                    SelectObjectives(faction, modeObjectives, 2, GetRandomObjectiveCount(maxMajor, minMajor)));
            }
        ...
        try
        {
            ...
            // foreach (var (uid, obj) in modeObjectives.Where(x => !x.Comp.Active && x.Comp.FactionNeutral))
            // {
            ...
            var neutralCandidates = modeObjectives.Where(x => x.Comp.FactionNeutral && !x.Comp.Active).ToList();
    ...
    // TODO: implement this to minimalize the Main() method duplications
    private (int? MinMinor, int MaxMinor, int? MinMajor, int MaxMajor) GetFactionObjectiveBounds(ObjectiveMasterComponent master, string faction)
    {
